...
Twitter.prototype.get = function(url, params, callback) {
  ...
  this.request = this.oauth.get(url + '?' + querystring.stringify(params),
  ...
  function(error, data, response) {
    if (error && error.statusCode) {
      ...
      callback(null, json, this.request);
    }
  });
  return this;
};
...
Twitter.prototype.stream = function(method, params, callback) {
  ...
  var stream_base = this.options.stream_base,
      ...
      reconnect_queue = setTimeout(function () { }, 0);
      
  function linear_backoff (backoff) {
    if (backoff.current < backoff.max) {
      backoff.current += backoff.min;
    }
  }
  
  function exponential_backoff (backoff) {
    if (backoff.current < backoff.max) {
      backoff.current = backoff.current * 2;
    }
  }
  
  function reset_backoffs () {
    http_backoff.current = http_backoff.min;
    network_backoff.current = network_backoff.min;
    rate_http_backoff.current = rate_http_backoff.min;
  }
  
  function try_queue_reconnect (backoff) {
    if (reconnect_queue && (((reconnect_queue._idleStart.getTime() + reconnect_queue._idleTimeout) - new Date().getTime()) < backoff.current)) {
      reconnect_queue = setTimeout(stream.request_stream, backoff.current);
    }
  }
  ...
  stream.destroySilent = function() {
    if ( typeof stream.request.abort === 'function' )
      stream.request.abort(); // node v0.4.0
    else
      stream.request.socket.destroy();
  };
  ...
  stream.request_stream = function () {
  
      stream.request = self.oauth.post(
        url,
      ...
      stream.request.on('response', function(response) {
        ...
        if(response.statusCode > 200) {
          ...
          if (response.statusCode == 420) {
            try_queue_reconnect(rate_http_backoff);
            exponential_backoff(rate_http_backoff);
          }
          else {
            try_queue_reconnect(http_backoff);
            exponential_backoff(http_backoff);
          }
          console.error(res_codes.stream[response.statusCode]);
          stream.emit('error', 'http', response);
        }
        else
        {
          reset_backoffs();
          ...
          response.on('error', function(error) {
            try_queue_reconnect(network_backoff);
            linear_backoff(network_backoff);
            stream.emit('error', 'stream', error);
          });
          response.on('end', function() {
            try_queue_reconnect({ current : 5000});
      ...
      stream.request.on('error', function(error) {
        linear_backoff(network_backoff);
        try_queue_reconnect(network_backoff);
        stream.emit('error', 'request', error);
      });
      stream.request.end();
...
Twitter.prototype.cookie = function(req) {
  ...
  var twauth = null;
  try {
    twauth = JSON.parse(getState(req, this.options.cookie));
  } catch (error) {
    twauth = null;
  }
  return twauth;
  ...
  return function handle(req, res, next) {
    ...
    if ( !self.oauth._authorize_callback ) {
      ...
      var scheme = (req.socket.secure || self.options.secure) ? 'https://' : 'http://',
        ext_path = url.parse(scheme + req.headers.host + req.url, true);
      self.oauth._authorize_callback = ext_path.href;
    }
    ...
    // Begin OAuth transaction if we have no cookie or access_token_secret
      ...
      function(error, oauth_token, oauth_token_secret, oauth_authorize_url, params) {
        if ( error ) {
          ...
          res.writeHead(302, {
            'Location': self.options.authorize_url + '?' + querystring.stringify({oauth_token: oauth_token})
          });
          res.end();
          return;
        }
...
Twitter.prototype.userProfileImage = function(id, params, callback) {
  ...
  request.on('response', function(response) {
    ...
    if (!response.headers.location) { 
      callback(new Error('HTTP Error ' + response.statusCode + ': ' + http.STATUS_CODES[response.statusCode]));
    }
...
Twitter.prototype.getBlockList = function(params, callback) {

  var url = '/blocks/list.json';
  this._getUsingCursor(url, params, callback);
  return this;
};
...
Twitter.prototype.getContributors = function(params, callback){
  var url = '/users/contributors.json';
  this.get(url, params, callback);
  return this;
};

Twitter.prototype.getContributees = function(params, callback){
  var url = '/users/contributees.json';
  this.get(url, params, callback);
  return this;
};
...
Twitter.prototype._getUsingCursor = function(url, param, callback) {
  var self = this,
    params = param || {},
    key = params.key || null,
    result = [];
